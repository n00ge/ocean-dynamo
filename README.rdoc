...
* +has_many+ now works, in a minimalist fashion. To implement the rest of the interface,
  we need association proxies.

* Instances of classes with one or more +has_many+ relations now save all loaded relations 
  after the instance has been saved. To save only modified association collections, we need
  association proxies. Collections which are +nil+ are not saved and leave their children
  untouched. Collections which are +[]+ are saved, which means they destroy all their children. This difference is crucial, until we get association proxies.

* #reload clears all cached children.
