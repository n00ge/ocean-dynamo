...
+dynamo_schema+ takes args and many options. Here's the full syntax:
 ...
 Tip: you might want to use the following idiom when declaring a +dynamo_schema+. It is used everywhere in ocean-rails:
...
This will auto-create the DynamoDB table in all environments except production, and it will also add a suffix to the table name so tests never collide. Cf +Api.basename_suffix+ for more information.
