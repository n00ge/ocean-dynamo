...
* belongs_to can now be specified only once in each class (since it claims the range key to
  store its UUID and the hash key to store the UUID of the parent).
